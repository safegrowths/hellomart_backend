...
router.post('/order_list', verifyToken, async (req, res) => {
  try {
    const user_id = req.body.user_id || req.body.id || req.user.id;
    const { order_id } = req.body;
    ...
    let sql = `
      SELECT 
        ...
        o.allamount AS allamountss,
        o.user_id AS userids,
        o.paymode AS payment_mode,
        v.vendorShopName,
        p.*,
        a.locality
      FROM orders o
      ...
      if (!groupedOrders[orderId]) {
        groupedOrders[orderId] = {
          orderDetail: {
            ...
            allamount: row.allamountss,
            payment_mode: row.payment_mode,
            adress_id: row.locality,
        ...
        for (const item of cartItems) {
          ...
          let vendorAttr = [];
          try {
            vendorAttr = JSON.parse(item.vendor_productattribute || "[]");
          } catch (e) {}

          for (const attrId of attributeIds) {
            let matched = vendorAttr.find(a => a.id == attrId);

            if (matched) {
              let quantity = parseInt(item.quantity);
              let mrp = parseFloat(matched.price);
              let off = parseFloat(matched.discount_percentage || 0);

              let price = mrp - (mrp * off / 100);
              let images = matched.product_images || [];
        ...
        groupedOrders[orderId].products = arrs;
      }
    }
...
router.post('/order_history', verifyToken, async (req, res) => {
  try {
    const user_id = req.body.user_id || req.body.id || req.user.id;
    const { order_id } = req.body;
...
router.get('/products_off', async (req, res) => {
  try {
    ...
    // ✅ Filter only 40% discount वाले products
    const filteredProducts = rows.map(product => {
      ...
      try {
        if (product.productattribute) {
          ...
          // ✅ Only 40% discount
          parsedAttributes = parsedAttributes.filter(
            item => Number(item.discount_percentage) === 40
          );
      ...
      // ✅ Return only those products having 40% discount
    ...
    return res.status(200).json({
      status: 200,
      msg: "40% OFF products fetched successfully",
